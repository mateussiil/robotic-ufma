...
class Env:  # Environment class
    ...
    def write_info(self, x, y, theta, gamma):
        Pose = f"x = {x}; y = {y}; theta = {int(math.degrees(theta))}; gamma = {math.degrees(gamma)}"
    ...
    def move(self, event=None):
        if event is not None:
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_q:  # Increase gamma
                    self.gamma += 0.0001*self.m2p
                elif event.key == pygame.K_a:  # Decrease gamma
                    self.gamma -= 0.0001*self.m2p
